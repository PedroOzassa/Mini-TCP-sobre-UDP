...
from utils.packet import (
    ...
    TYPE_ACK,
    TYPE_NAK,
    make_data_gbn as make_data,
    make_ack_gbn as make_ack,
    decode_packet_gbn as decode_packet,
    is_ack_gbn as is_ack,
)
...
class GBNSender:
    ...
    def send(self, data: bytes):
        """Send a single application message using sliding window logic."""
        ...
        while seq in self.send_buffer:
            # Either process ACKs or handle timeout
    ...
    def _on_timeout(self):
        """Timeout handler: retransmit all packets in the window."""
...
class GBNReceiver:
    ...
    def start(self):
        # Start the receiver loop in a daemon thread
    ...
    def _run(self):
        while True:
            ...
            # Corrupted -> send duplicate ACK for last good
            if not pkt["checksum_ok"]:
            ...
            # Out-of-order packet -> duplicate ACK for last good
            last_good = (self.expectedseqnum - 1) & 0xFFFFFFFF
            ack = make_ack(last_good)
            self.channel.send(ack, self.sock, addr)
